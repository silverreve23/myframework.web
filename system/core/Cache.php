...
use system\core\{ExClassCore, View};
use system\libs\LA;
...
final class Cache extends ExClassCore{
	...
	public static function isCache($name_f = null){
		...
		if(!empty($name_f)){
			...
			if(file_exists($tmp_cache_file)){
				...
				$cache_array = LA::getArrayFromFile(self::$_tmp_cache);
				$arr_cache_compare = ['cache:enable', 'cache:time', 'cache:list'];

				if(!LA::getKeysExists($arr_cache_compare, $cache_array))
